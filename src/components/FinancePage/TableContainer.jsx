 ...
 /*   if(tableData !== null){
  ...
  useEffect(() => {
    ...
    const fetchTableData = async () => {
      try {
        const res = await axios.post(`https://aarnainfra.com/ladder2/client/fetch2.php?page=${currentPage}`, mainRequest);
  ...
  return (
    <>
      <div
        ...
        } max-h-[63vh] rounded-t-lg overflow-auto tableContainer`}
      >
        <table className="w-full border mt-0">
          <thead className="h-14 bg-[#F7F8FF] sticky top-[-1%] mt-0 z-10">
          <tr>
           <th className="text-[#9A55FF] font-medium text-xs text-start pl-2 pr-2 min-w-[5vw]">
             Name
           </th>
                ...
                <td className="text-center text-xs">
                  ...
                  <td className="text-center text-xs">
                  {
                    item?.fetched_brokerage_percent === item?.fetched_ladder_percent
                      ? 0
                      : item?.fetched_ladder_percent !== null
