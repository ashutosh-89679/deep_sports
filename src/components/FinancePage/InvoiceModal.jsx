...
const InvoiceModal = ({ clientID, clientName , onClose }) => {
  ...
  const getData = async () => {
    try {
      const res = await axios.post(
        `https://aarnainfra.com/ladder2/client/getInvoiceDetails.php`,
        { client_id: clientID }
      );
  ...
  const getEstimation = async () => {
    try {
      const res = await axios.get(
        `https://aarnainfra.com/ladder2/client/company_estimation_micro.php`,
      );
  ...
  useEffect(() => {
    if (company_data && company_data.length > 0) {
        const calculatedDate = calculateDynamicDate(givenDate, company_data[0].id.toString(), estimationData);
  ...
  const convertInvoice = (clientID, invoiceValue, company,  invoiceType, raiseDate) => {
    ...
    function promptForValidDate() {
      ...
      if (inputDate === null) {
        ...
        axios
          .put(`https://aarnainfra.com/ladder2/client/invoice.php`, {
            client_id: clientID,
          ...
          .then((response) => {
            ...
            toast.success("Invoice Coverted Successfully");
  ...
  const updateData = async (client_id, invoice_no , invoice_id) => {
    ...
    if(type !== null && givenDate !== null){
      ...
      if (type === '2' && expectedDate === null) {
        ...
        try {
          const res = await axios.post(
            'https://aarnainfra.com/ladder2/client/invoice.php',
            requestBody
          );
