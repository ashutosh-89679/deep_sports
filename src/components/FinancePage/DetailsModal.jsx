...
const DetailsModal = ({ detailsData, clientName,  onClose }) => {
 ...
 const { activeUserData } = useContext(AppContext);
  ...
  let invoiceValue = calculateInvoiceValue(agreementValue, detailsData?.fetched_brokerage_percent);
if(detailsData?.fetched_ladder_percent !== null){
  invoiceValue =  calculateInvoiceValue(agreementValue, detailsData?.fetched_ladder_percent);
}
/*   if(detailsData?.fetched_kicker_percent !== "0" && detailsData?.fetched_ladder_percent !== null ){
  invoiceValue =  (parseInt(agreementValue) * (Number(detailsData?.fetched_ladder_percent) + Number(detailsData?.fetched_kicker_percent) / 100));
}   */
  ...
  const realizeAmount = totalInvoiceValue -
    TDSAmount 
      -
    (detailsData?.generic_details?.cashback_amount || 0)  ;

 
/*     if (detailsData?.kicker_value !== null && detailsData?.kicker_value !== '0') {
    totalInvoiceValue = invoiceValue + GSTAmount + Number(detailsData?.kicker_value);
  } else { 
    totalInvoiceValue = invoiceValue + GSTAmount; 
  }  */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleUpdate = async () => {
      ...
      if (
        ...
        if (confirmed) {
          try {
            const response = await axios.patch('https://aarnainfra.com/ladder2/client/main.php', {
        ...
        <div className="flex gap-4 mt-6">
          <div className="flex-1 relative border border-[#E0E0E0] rounded h-9 z-40">
            {/* <input
              type="text"
              value={
                sourceByAndCloseBy[detailsData?.generic_details?.sourced_by]
              }
              disabled
          ...
          {accordion?.costDetails && (
            <>
              ...
              <div className="flex gap-4 mt-6">
                <div className="flex-1 relative border border-[#E0E0E0] rounded h-9 ">
                  <input
                    value={detailsData?.fetched_ladder_percent ? (detailsData?.fetched_ladder_percent - detailsData?.fetched_brokerage_percent).toFixed(2) : 0}
                    placeholder="Enter Here.."
                    type="text"
                    disabled
                    className="pt-1 rounded outline-none text-sm text-[#838383] pl-3 h-full w-full"
