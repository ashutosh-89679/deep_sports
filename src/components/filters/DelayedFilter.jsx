...
const FilterDropdown = ({
  ...
  const [companyNameArray , setCompanyNameArray] = useState([]);
    ...
    /* function getFinancialPeriodDates(year, binary) {
      ...
      if (binary.startsWith('Q')) {
          const quarter = parseInt(binary[1]);
          switch (quarter) {
              case 1:
                  startDate = new Date(fullStartYear, 3, 1); // April 1st
                  endDate = new Date(fullStartYear, 5, 30); // June 30th
                  break;
              case 2:
                  startDate = new Date(fullStartYear, 6, 1); // July 1st
                  endDate = new Date(fullStartYear, 8, 30); // September 30th
                  break;
              case 3:
                  startDate = new Date(fullStartYear, 9, 1); // October 1st
                  endDate = new Date(fullStartYear, 11, 31); // December 31st
                  break;
              case 4:
                  startDate = new Date(fullEndYear, 0, 1); // January 1st
                  endDate = new Date(fullEndYear, 2, 31); // March 31st
                  break;
              default:
                  return "Invalid quarter";
          }
      } else if (!isNaN(parseInt(binary)) && parseInt(binary) >= 1 && parseInt(binary) <= 12) {
          // Adjust the month number according to the new logic
          let adjustedMonthNumber = parseInt(binary) + 3;
          if (adjustedMonthNumber > 12) {
              adjustedMonthNumber -= 12; // Wrap around if the month number exceeds 12
          }
          const monthIndex = adjustedMonthNumber - 1; // Adjust for 0-based indexing
          startDate = new Date(monthIndex < 3 ? fullEndYear : fullStartYear, monthIndex, 1);
          endDate = new Date(monthIndex < 3 ? fullEndYear : fullStartYear, monthIndex + 1, 0); // 0th day of next month is last day of current month
      } else {
...
//console.log(requestBody);  
  const getData = async (requestData) => {
   // console.log(requestData);
    axios
      .post("https://aarnainfra.com/ladder2/client/cashflow/delayed_cashflow_test.php", requestData) 
      .then((res) => {
        console.log(res);
        if(res?.data){
