...
const SortIconComponentAging = (
  ...
  const {  updateSortByAgingRefBody } = useStore();
  const dropdownRef = useRef(null);

  const handleSortClick = (type, typeName) => {
    const sortData = { type, typeName };
    updateSortByAgingRefBody(sortData);
      ...
      {isDropdownVisible && (
        <div className="absolute top-7 drop-shadow-lg z-20 w-[195px] rounded  bg-white p-2 text-sm font-normal ">
           <p
            ...
            onClick={() => handleSortClick("type" , "expected_date_sort")}
          >
              Sort By : Expected Date
          </p>
          <p
            className="cursor-pointer leading-none py-2 hover:bg-[#EADCFF] hover:text-[#000000] rounded pl-3 text-[#8B8B8B] "
            onClick={() => handleSortClick("type", "max_age_sort")}
          >
             Sort By : Max Aging Days
          </p>
          <p
            className="cursor-pointer leading-none py-2 hover:bg-[#EADCFF] hover:text-[#000000] rounded pl-3 text-[#8B8B8B] "
            onClick={() => handleSortClick("type", "invoice_raise_sort")}
          >
             Sort By : Invoice Raise Date
          </p>
        </div>
      )}
    </div>
...
export default SortIconComponentAging;
